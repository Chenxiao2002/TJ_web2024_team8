...
def login(request):
    try:
        ...
        if user:
            if user.status==2:
                return JsonResponse({'error': '黑名单用户，无法登录'}, status=402)
...
def update_avatar(request, payload):
    try:
        ...
        user_id = payload['user_id']
        file_path = str(BASE_DIR) + '/media/avatar/' + str(user_id) + '-' + file.name
        check_and_delete(id=user_id, mainPath=str(BASE_DIR) + '/media/avatar')
        with open(file_path, 'wb') as destination:
            for chunk in file.chunks():
                destination.write(chunk)
        result = {
            'filename': file.name,
            'filepath': 'http://123.60.149.233:8000/media/avatar/' + str(user_id) + '-' + file.name,
        }
...
def user_control_index(request, payload):
    try:
        ...
        if user:
            user_data = []
            if types == 'posts':
                user_data = Post.objects.filter(uid=user_id).all()
